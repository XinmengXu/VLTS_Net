...
from vlts_net.datas.transform import get_preprocessing_pipelines
from vlts_net.videomodels import ResNetVideoModel
...
if __name__ == '__main__':
    ...
    train_conf["main_args"].setdefault(
        "exp_dir",
        os.path.join(os.getcwd(), "Experiments", "checkpoint", train_conf["exp"]["exp_name"]),
    )
    
    # Load model
    # print(["main_args"]["exp_dir"])
    checkpoint_path = os.path.join(train_conf["main_args"]["exp_dir"], "best_model.pth")
    audiomodel = VLTS.from_pretrain(checkpoint_path, sample_rate=train_conf["datamodule"]["data_config"]["sample_rate"], **train_conf["audionet"]["audionet_config"])
    ...
    with torch.no_grad():
        for i in range(number_of_speakers):
            ...
            mix = torchaudio.load(output_path+"audio.wav")[0].to(device)
            
            mouth_emb = videomodel(torch.from_numpy(mouth_roi[None, None]).float().to(device))
            est_outputs = audiomodel(mix[None], mouth_emb)
